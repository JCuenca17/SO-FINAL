...
def abrir_aplicacion2():
    ...
    botones = [
        ("Paginación", lambda: ejecutar_script("main.py")),
        ("Segmentación", lambda: ejecutar_script("main.py")),
        ("Segmentación Paginada", lambda: ejecutar_script("main.py"))
    ]
...
def abrir_aplicacion3():
    ventana3 = tk.Toplevel(root)
    ventana3.title("Gestión de Concurrencias")
    ventana3.geometry("300x300")

    etiqueta = tk.Label(ventana3, text="Elija el tipo de algoritmo", font=("Helvetica", 12))
    etiqueta.pack(pady=10)

    botones = [
        ("Monitor", lambda: ejecutar_script("monitor_procesos.py")),
        ("Semáforo", lambda: ejecutar_script("semaforos.py"))
    ]

    for texto, comando in botones:
        boton = tk.Button(ventana3, text=texto, command=comando, width=20, height=2)
        boton.pack(pady=5)
...
img_logo3 = tk.PhotoImage(file="concurrencia.png")

# Crear los botones de las aplicaciones con imágenes
...
boton_app3 = tk.Button(escritorio, image=img_logo3, bg="lightblue", command=abrir_aplicacion3, width=64, height=64)
...
etiqueta_boton3 = tk.Label(escritorio, text="Concurrencia", bg="lightblue", font=("Helvetica", 10))
...
boton_app3.place(x=100, y=300)  # Ajuste para que el botón 3 esté debajo del botón 2

# Posicionar las etiquetas debajo de los botones
etiqueta_boton1.place(x=100, y=160)
etiqueta_boton2.place(x=100, y=260)
etiqueta_boton3.place(x=100, y=360)  # Ajuste para que la etiqueta del botón 3 esté debajo del botón 3
